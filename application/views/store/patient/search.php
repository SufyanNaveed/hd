                ...
                <div class="box box-primary">
                    <div class="box-header with-border">
                        ...
                        <div class="box-tools addmeeting">
                                <a type="button" href="<?php echo base_url('admin/patient/opd_list')?>" name="search" value="export_filter" class="btn btn-warning btn-sm checkbox-toggle" style="" autocomplete="off"><i class="fa fa-search"></i> Export</a>
                                <a data-toggle="modal" href="<?php echo base_url() ?>hospital/patient/ipdPatientCreate" id="addp" class="btn btn-primary btn-sm addpatient"><i class="fa fa-plus"></i>  <?php echo $this->lang->line('add') . " " . $this->lang->line('patient') ?></a>
                        ...
                        <div class="box-body">
                            <!-- Filter Controls -->
                            <div class="row mb-3" style="background-color: #f8f9fa; padding: 15px; border-radius: 5px; margin-bottom: 20px;">
                                <div class="col-md-12">
                                    <h5 style="margin-bottom: 15px; color: #333;"><i class="fa fa-filter"></i> Filter Options</h5>
                                </div>
                                <div class="col-md-3">
                                ...
                                <div class="col-md-3">
                                    <div class="form-group">
                                        <label for="medication_filter"><i class="fa fa-medkit"></i> Medication Assignment:</label>
                                        <select class="form-control" id="medication_filter" name="medication_filter">
                                            <option value="">All Patients</option>
                                            <option value="assigned">With Medication Assigned</option>
                                            <option value="not_assigned">Without Medication Assigned</option>
                                        </select>
                                    </div>
                                </div>
                                <div class="col-md-3">
                                    <div class="form-group">
                                        <label>&nbsp;</label>
                                        <div>
                                            <button type="button" class="btn btn-info btn-sm" id="apply_filters" style="margin-right: 5px;">
                        ...
                        <form id="formadd" accept-charset="utf-8" action="<?php echo base_url() . "admin/patient" ?>" enctype="multipart/form-data" method="post">
                                    ...
                                    <div class="row">
                                        <div class="col-sm-12">
                                            <div class="form-group">
                                                ...
                                                <input id="admission_date" <?php if(!isset($this->session->userdata['hospitaladmin']['roles']['Super Admin'])){ echo 'readonly';}?> name="appointment_date" placeholder="" type="text" class="form-control datetime" />
                            ...
                            <div class="row">
                                <div class="box-footer">
                                    ...
                                    <div class="pull-right print_prescription" style="margin-right: 10px; ">
                                    <button type="button"  class="btn btn-info pull-right printprescriptionmanual"  onclick="printprescriptionmanual()" >
                                    Print Prescp</button>
                                    ...
                                    <div class="row">
                                        <div class="col-sm-12">
                                            <div class="form-group">
                                                <label><?php echo $this->lang->line('appointment') . " " . $this->lang->line('date'); ?></label>
                                                <small class="req">*</small>
                                                <input id="revisit_date" <?php if(!isset($this->session->userdata['hospitaladmin']['roles']['Super Admin'])){ echo 'readonly';}?> name="appointment_date" placeholder="" type="text" class="form-control datetime"   />
                                                <span class="text-danger"><?php echo form_error('appointment_date'); ?></span>
                                            </div>
                                        </div>
                                        <div class="col-sm-6">
        ...
        success: function (res) {
            if (res) {
                ...
                $.each(res, function (i, obj) {
                    ...
                    // console.log("Doctor Name: " + obj.name);

                    if (id == 24 && obj.name == 'Casualty on Duty MO') {
                        sel = "selected";
                    }
                ...
                // Manually trigger change event for select2 to update for emergency
                $("#consultant_doctor").trigger('change');
...
<script type="text/javascript">
    $(document).ready(function() {
        var test_ajax = $('.test_ajax').DataTable({ // Assigned to a variable for global access
            "processing": true,
            "order": [[ 1, "desc" ]],
            "serverSide": true,
            "columns": [
                { "data": 0 }, // Sr. No.
                { "data": 1 }, // Name of Patient
                { "data": 2 }, // MR No.
                { "data": 3 }, // Invoice No.
                { "data": 4 }, // CNIC
                { "data": 5 }, // Gender
            ...
            "ajax": {
                "url": base_url+"hospital/patient/opd_search",
                "type": "POST",
                "data": function(d) {
        ...
        $("#apply_filters").on('click', function() {
            ...
            $(this).html('<i class="fa fa-spinner fa-spin"></i> Applying...');
            $(this).prop('disabled', true);
            
            // Reload the DataTable with filters
            if (typeof test_ajax !== 'undefined' && test_ajax) {
                test_ajax.ajax.reload(function() {
                    // Reset button after reload
                    $("#apply_filters").html('<i class="fa fa-filter"></i> Apply Filters');
                    $("#apply_filters").prop('disabled', false);
                });
            } else {
                // Reset button if DataTable not available
                $("#apply_filters").html('<i class="fa fa-filter"></i> Apply Filters');
                $("#apply_filters").prop('disabled', false);
            }
        });
        ...
        $("#clear_filters").on('click', function() {
            ...
            if (typeof test_ajax !== 'undefined' && test_ajax) {
                test_ajax.ajax.reload(function() {
                    // Reset button after reload
                    $("#clear_filters").html('<i class="fa fa-times"></i> Clear');
                    $("#clear_filters").prop('disabled', false);
                });
            } else {
                // Reset button if DataTable not available
                $("#clear_filters").html('<i class="fa fa-times"></i> Clear');
                ...
                success: function (data) {
                    ...
                    if (data.status == "fail") {
                        ...
                        $.ajax({
                            ...
                            type: 'POST',
                            data: {payslipid: data.id, print: 'yes'},
                            success: function (result) {
                                $("#testdata").html(result);
                                popup(result);
                            }
