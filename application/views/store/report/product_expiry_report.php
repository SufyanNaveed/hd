        ...
        <div class="row">
            <div class="col-md-12">
                <div class="box box-primary">
                    ...
                    <form role="form" action="<?php echo site_url('hospital/report/productExpiryReport') ?>" method="post" class="" onsubmit="return validateForm(event)">
                            ...
                            <table class="custom-table table table-striped table-bordered table-hover example">
                                <thead>
                                    <tr>
                                        ...
                                        <th>Approved Qty</th>
                                        <th>Rate</th>
                                        <th class="text-right">Total (<?php echo $currency_symbol; ?>)</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <?php
                                    if (empty($resultlist)) {
                                        ...
                                        foreach ($resultlist as $bill) {
                                            $approved_qty = $bill['total_quantity'] ?? 0;
                                            // $rate = is_numeric($bill['total_amount']) ? (float)$bill['total_amount'] : 0;
                                            $rate = is_numeric($bill['purchase_price']) ? (float)$bill['purchase_price'] : 0;
                                            $amount = $approved_qty * $rate;
                                            $total += $amount;
                                            ?>
                                            <tr>
                                ...
                                <?php if (!empty($resultlist)) { ?>
                                    <tr class="box box-solid total-bg">
                                        ...
                                        <td class="text-right"><strong><?php echo $currency_symbol . number_format($total, 2); ?></strong></td>
                                    </tr>
                                <?php } ?>
                            </table>
