...
class Consultcharges extends Admin_Controller
{
    ...
    public function addconsultcharges()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $data = array(
                ...
                'percentage_amount' => $this->input->post('doctor_percentage'),
            );
    ...
    public function getDetails()
    {
        ...
        $result['all_staff']       = $this->charge_model->getConsultantStaffCharges($id);
    ...
    public function emg()
    {
        ...
        $this->session->set_userdata('top_menu', 'setup');
        $this->session->set_userdata('sub_menu', 'charges/index');
        $data['resultlist'] = $this->charge_model->get_chargedoctorfee('emg');
        $data['schedule']   = $this->organisation_model->get();
        $this->load->view("layout/header");
        $this->load->view("admin/consultcharges/emg_charge", $data);
        $this->load->view("layout/footer");
    }
    public function addEmgConsultantCharges()
    {
        if (!$this->rbac->hasPrivilege('doctor_opd_charges', 'can_add')) {
            access_denied();
        }
    ...
    {
        if($update=='update'){
            $this->db->delete('consult_charges_percentage', array('charges_id' => $chargesID));
        }
        ...
        if($this->input->post('staff')!==''){
            ...
            $this->db->insert_batch('consult_charges_percentage',$staffData);
        }
    }
