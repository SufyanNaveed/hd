...
class Pharmacy extends Admin_Controller
{
    ...
    public function search($stock = null)
    {
        if (!$this->rbac->hasPrivilege('medicine', 'can_view')) {
            access_denied();
        }
        $search_type                = $_POST["search_type"] ?? "this_week";
    ...
    public function dt_search($stock = null)
    {
        ...
        if (!empty($_POST['hospital_id'])) {
            $where_condition['hospital_id'] = $_POST['hospital_id'];
        }
        
        $resultlist   = $this->pharmacy_model->search_datatable($where_condition, $stock,$search_type);
        $total_result = $this->pharmacy_model->search_datatable_count($where_condition, $stock, $search_type);
        $data         = array();
        $total_qty = 0;
        $total_purchase = 0;
        $total_sale = 0;
        ...
        foreach ($resultlist as $result_key => $result_value) {
            ...
            $nestedData[] = $result_value->batch_no;
            $data[]       = $nestedData;
        }
