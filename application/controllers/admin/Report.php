...
class Report extends Admin_Controller
{
    ...
    public function storeyearlyconsumptionreport()
    {
        ...
        if ($this->input->post('search') == 'export_pdf') {
            ...
            $this->load->view('layout/header');
            $this->load->view('admin/report/store_yearly_consumption_report.php', $data);
            $this->load->view('layout/footer');
        }
    }
    ...
    public function storeYearlyConsumptionPdf($data)
    {
        ...
        if (empty($data['resultlist'])) {
            ...
            $html .= '<h1>Monthly Drug Consumption Summary (Yearly Overview Summary) (A month-wise summary of drug usage throughout the year)</h1>';
            $html .= '<div class="no-records">No records found</div>';

            $this->dompdf->set_paper("A4", "portrait");
            $this->dompdf->loadHtml($html);
            $this->dompdf->render();
            $this->dompdf->stream("Monthly Drug Consumption Summary (Yearly Overview Summary) (A month-wise summary of drug usage throughout the year).pdf", ["Attachment" => 1]);
            return;
        }
    ...
    public function storeConsumptionPdf($data)
    {
        ...
        if (empty($data['resultlist'])) {
            ...
            $html .= '<h1>Department-wise Drug Utilization Report (Detailed report of drugs consumed by each department)</h1>';
            $html .= '<div class="no-records">No records found</div>';
   ...
   public function outOFStockReport()
    ...
    {
        $this->session->set_userdata('top_menu', 'Reports');
        $this->session->set_userdata('sub_menu', 'admin/report/patientSummaryReport');
        $this->load->model('common_model');
        ...
        foreach($hospitals as $h=>$hospital){
            ...
            $wards[$h] = $this->db->get('main_stores')->result_array();
        } 

        $data = array();
        $count = 0;
        foreach($wards as $w=>$ward){ 

            foreach($ward as $sw=>$single_ward){
                ...
                $data[$count]['hospital_id'] = $hospitals[$w]['id'];
                $data[$count]['hospital_name'] = $hospitals[$w]['name'];
                $data[$count]['store_id'] = $single_ward['id'];
                $data[$count]['store_name'] = $single_ward['store_name']; 
                $data[$count]['national'] = Count($national) > 0 ? $national['national'] : 0;
                $data[$count]['other_national'] = Count($other_national) > 0  ?  $other_national['other_national'] : 0;
                $data[$count]['Male'] = Count($Male) > 0 ? $Male['Male'] : 0;
                $data[$count]['Female'] = Count($Female) > 0 ? $Female['Female'] : 0;
                $data[$count]['Other'] = Count($Other) > 0 ? $Other['Other'] : 0;
        ...
        $data['hospitals'] = $this->common_model->getRecord(null, 'hospitals', '');
        $data['main_stores'] = $this->common_model->getRecord(null, 'main_stores', '');
        //echo '<pre>'; print_r($data); exit;
        ...
        if ($this->input->post('search') == 'export_pdf') {
            ...
            $this->load->view('layout/header');
            $this->load->view('admin/report/patient_summary_report', $data);
            $this->load->view('layout/footer');
        }
    }

    public function patientMedicineReport()
    {
        $this->session->set_userdata('top_menu', 'Reports');
        $this->session->set_userdata('sub_menu', 'admin/report/patientSummaryReport');
        $this->load->model('common_model');
        ...
        foreach($wards as $w=>$ward){ 
            ...
            foreach($ward as $sw=>$single_ward){ 
                ...
                $this->db->select("COUNT(pharmacy_bill_detail.quantity) AS total_issued_medicines,
                ...
                $this->db->where('pharmacy_bill_basic.hospital_id', $single_ward['entity_id']);
                $this->db->where('pharmacy_bill_basic.store_id', $single_ward['id']);
                if(isset($start_date)  && !empty($start_date) && isset($end_date) && !empty($end_date)){
                ...
                $data[$count]['IPD'] = $IPD['IPD'];
                $data[$count]['OPD'] = $OPD['OPD'];
                $data[$count]['total_issued_medicines'] = $medicine['total_issued_medicines'];
                $data[$count]['total_issued_price'] = $medicine['total_issued_price'];
        ...
        if ($this->input->post('search') == 'export_pdf') {
            ...
            $this->load->view('layout/header');
            $this->load->view('admin/report/patient_medicine_report', $data);
            $this->load->view('layout/footer');
        }
    }
