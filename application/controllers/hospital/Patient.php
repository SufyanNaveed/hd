...
class patient extends Hospital_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model('prescription_model'); // Load the user model if not already loaded

        $this->load->model('medicine_dosage_model'); // Load the user model if not already loaded
        $this->load->model('emergency_icu_model'); // Load the user model if not already loaded
        ...
        $this->hospital_name = isset($user_details->hospital_name) ? $user_details->hospital_name : null;
    ...
    public function addIPDOPDPatient()
    {
        ...
        // $this->form_validation->set_rules('father_name', 'Father Name', 'trim|required|min_length[3]');
        $this->form_validation->set_rules('mobile', 'Mobile', 'trim|required|min_length[10]');
        // $this->form_validation->set_rules('cnic', 'CNIC', 'trim|required|regex_match[/^[0-9]{5}-[0-9]{7}-[0-9]$/]');
        ...
        if ($this->session->userdata()['hospital']['role'] === "Department Pharmacist") {
        ...
        // Check if patient exists before inserting a new one
        if (!$patient_id) {
            if ($cnic && $this->patient_model->check_cnic_exists($cnic)) {
                echo json_encode(['status' => 'error', 'message' => 'A patient with this CNIC already exists!']);
                return;
            }
    
            $check_patient_id = $this->patient_model->getMaxId();
            $patient_id = $check_patient_id ? $check_patient_id + 1 : 1;
            $patient_unique_id = $this->formatNumber($patient_id, 'mr');
            $is_ipd = $this->input->post('is_ipd') ?  $this->input->post('is_ipd') : '';
            ...
            $data = [
                ...
                'patient_cnic' => !empty($this->input->post('cnic')) ? $this->input->post('cnic') : null,
                'other_nationality' => $this->input->post('other_nationality'),
                ...
                'emergency_icu_id'=>$this->input->post('emergency_icu_id')
            ];
    
            // print_r($data);exit;
            // Insert new patient and get its ID
            $insert_id = $this->patient_model->add_patient($data);
        } else {
            $insert_id = $patient_id;
        }
        ...
        // Check if OPD visit already exists
        $existing_opd_id = $this->checkExistPatient($insert_id, $date, $this->hospital_id);
        ...
        $opd_no_id = $check_opd_id + 1;
        $setOPD = $this->formatNumber($opd_no_id, 'opd');
        ...
        $opd_data = [
            ...
            'hospital_id' => $this->hospital_id,
            'store_id'=>$this->store_id,
            'opd_slip_no'=>$this->input->post('opd_slip_no'),
        ...
        if (!$this->patient_model->check_patient_exists($insert_id)) {
            $this->patient_model->add_patient(['id' => $insert_id]);
        }
        ...
        // Fetch the highest token number for the given date
        $this->db->select_max('token_number');
        $query = $this->db->get('daily_patient_tokens');
        $result = $query->row();
    
        // Assign the next token number
        $token_data['token_number'] = ($result && $result->token_number) ? $result->token_number + 1 : 1;
    
        // Insert token data
        $this->db->insert('daily_patient_tokens', $token_data);
    ...
    public function add_inpatient()
    {
        ...
        if (!$patient_id) {
            ...
            $patient_id = $check_patient_id ? $check_patient_id + 1 : 1;
                       $patient_unique_id = $this->hospital_id . '-' . $this->formatNumber($patient_id, 'mr');
            $is_ipd = $this->input->post('is_ipd') ?  $this->input->post('is_ipd') : '';
    ...
    public function addIPDPatient()
    {
        ...
        $this->db->insert('daily_patient_tokens', $token_data);
            echo json_encode([
                'status' => 'success',
                'message' => 'Visit already exists, no new visit added.',
                'patient_id' => $insert_id,
                'opd_id' => $existing_opd_id,
                'date'=>         date('Y-m-d', strtotime($appointment_date))
    ...
    public function add_revisit()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            ///////////////////////Comissio set///////////////////////////////////
            $chargesCommission = $this->staff_model->consultantChargesStaffCommission($this->input->post('consultant_doctor'));
            if (!empty($chargesCommission)) {
                $commission_month = date('m', strtotime($date));
                $commission_year = date('Y', strtotime($date));
                if ($chargesCommission['doctor_percentage_type'] == 'fixed') {
                    $comission_amount = $chargesCommission['percentage_amount'];
                } else {
                    $comission_amount = ($this->input->post('amount') * $chargesCommission['percentage_amount']) / 100;
                }

                $commission_data = array(
                    'staff_id' => $this->input->post('consultant_doctor'),
                    'bill_no' => $opd_id,
                    'appointment_date' => date('Y-m-d H:i:s', strtotime($date)),
                    'comission_month' => $commission_month,
                    'comission_year' => $commission_year,
                    'comission_amount' => $comission_amount,
                    'commission_type' => 'OPD',
                    'commission_percentage' => $chargesCommission['percentage_amount'],
                    'total_amount' => $this->input->post('amount'),

                );
                $this->db->insert('monthly_comission', $commission_data);
                $getStaffCommissions = $this->staff_model->getConsultantStaffCharges($chargesCommission['id']);
                if (!empty($getStaffCommissions)) {
                    ...
                    foreach ($getStaffCommissions as $sc) {
                        if ($sc['percentage_type'] == 'fixed') {
                            ...
                            $comission_amount = ($this->input->post('amount') * $sc['staff_percentage']) / 100;
                        }
                        $allstaffCommission[] = array(
    ...
    public function opdNotification($patient_id = '', $doctor_id, $opd_no = '', $url, $date)
    {
        ...
        if (!empty($patient_id)) {
            $notification_data = array(
                'notification_title' => $this->lang->line('notification_opd_visit_created'),
    ...
    public function opdpresNotification($patient_id = '', $doctor_id, $opd_no = '', $opd_no_value = '', $url, $visible_module)
    {
        ...
        if (!empty($patient_id)) {
            $notification_data = array(
                'notification_title' => $this->lang->line('notification_opd_prescription_created'),
                'notification_desc'                             => $patient_desc,
                'notification_for'                              => 'Patient',
                'notification_type'                             => 'opd',
                'receiver_id'                                   => $patient_id,
    ...
    public function ipdpresNotification($patient_id = '', $doctor_id, $ipd_no = '', $ipd_no_value = '', $url, $visible_module, $pres_id = '')
    {
        ...
        if (!empty($patient_id)) {
            $notification_data = array(
                'notification_title' => $this->lang->line('notification_ipd_prescription_created'),
                'notification_desc'                             => $patient_desc,
                'notification_for'                              => 'Patient',
    ...
    public function ipdNotification($patient_id = '', $doctor_id, $ipdno = '', $url, $date)
    {
        ...
        if (!empty($patient_id)) {
            $notification_data = array(
                'notification_title' => $this->lang->line('notification_ipd_visit_created'),
                'notification_desc'                             => $patient_desc,
                'notification_for'                              => 'Patient',
                'notification_type'                             => 'ipd',
                'receiver_id'                                   => $patient_id,
    ...
    public function addpatient()
{
    ...
    $this->form_validation->set_rules('mr_no', 'MR#', 'trim|required|xss_clean|is_unique[patients.mrno]');
    $this->form_validation->set_rules('patient_cnic', 'Patient CNIC', 'trim|regex_match[/^\d{5}-\d{7}-\d$/]|xss_clean');
    $this->form_validation->set_rules('mobileno', $this->lang->line('phone'), 'trim|required|regex_match[/^\d{4}-\d{7}$/]|xss_clean');
    ...
    if ($this->form_validation->run() == false) {
        ...
        // Manually check if mr_no exists
        $mr_no = $this->input->post('mr_no');
        $existing_mr_no = $this->db->where('mrno', $mr_no)->count_all_results('patients');

        if ($existing_mr_no > 0) {
            $array = array('status' => 'fail', 'error' => array("mr_no" => "MR# already exists!"), 'message' => '');
            echo json_encode($array);
            exit;
        }
        ...
        $cnic = $this->input->post('patient_cnic') ? $this->input->post('patient_cnic') : null;
        ...
        $comments = "New patient added: Name - " . $this->input->post('name') . ", Father Name - " . $this->input->post('father_name') . ", MR# - " . $patient_unique_id;

        $array = array('status' => 'success', 'message' => $this->lang->line('success_message'), 'id' => $insert_id);
    }
    echo json_encode($array);
}
    ...
    public function index()
    {
        ...
        $this->load->view('store/patient/patients.php', $data);
        $this->load->view('layout/user/footer');
    }
    

    public function all_patient_search()
    {
        ...
        $resultlist   = $this->patient_model->search_datatable_all($where_condition,$this->hospital_id,$this->store_id);
        $total_result = $this->patient_model->search_datatable_count($where_condition,$this->hospital_id,$this->store_id);
        ...
        foreach ($resultlist as $result_key => $result_value) {
            ...
            $action .= "<a href=" . base_url() . 'hospital/patient/profile/' . $result_value->pid . " class='btn btn-default btn-xs'  data-toggle='tooltip' title='" . $this->lang->line('show') . "'><i class='fa fa-reorder' aria-hidden='true'></i></a>";

            if (!$result_value->moved_to_ipd) {
                    $action .= "<a href=" . base_url() . 'hospital/patient/moveipd/' . $result_value->pid . " data-toggle='tooltip' onclick='return confirm('" . $this->lang->line('move') . " " . $this->lang->line('patient') . " " . $this->lang->line('in') . " " . $this->lang->line('ipd') . ")' data-original-title='" . $this->lang->line('move') . " " . $this->lang->line('in') . " " . $this->lang->line('ipd') . "' class='btn btn-default btn-xs' ><i class='fas fa-share-square'></i></a>";
            }
            // if ($this->rbac->hasPrivilege('revisit', 'can_add')) {
            ...
            $action .= "</div'>";
            $first_action = "<a href=" . base_url() . 'hospital/patient/profile/' . $result_value->pid . ">";
            $nestedData   = array();
            $nestedData[] = $i;
            ...
            $nestedData[] = $result_value->moved_to_ipd ? 'Yes' : 'No';
    ...
    public function search()
    {
        ...
        // $data['organisation']   = $this->organisation_model->get();
        $this->load->view('layout/user/header');
        $this->load->view('store/patient/search.php', $data);
        $this->load->view('layout/user/footer');
    }
    ...
    public function opd_search()
    {
        ...
        if (!empty($_POST['medication_filter'])) {
            $filters['medication_filter'] = $_POST['medication_filter'];
        }

        $resultlist   = $this->patient_model->search_datatable($where_condition,$this->hospital_id,$this->store_id,$filters);
        $total_result = $this->patient_model->search_datatable_count($where_condition,$this->hospital_id,$this->store_id,$filters);
    ...
    public function RefundTestReport($id, $status)
    {
        ...
        $this->patient_model->RefundTestReport($id, $status);
    }
    ...
    public function ipdsearch($bedid = '', $bedgroupid = '')
    {
        ...
        $this->load->view('layout/user/header');
        $this->load->view('store/patient/ipdsearch.php', $data);
        $this->load->view('layout/user/footer');
    }
    ...
    public function ipd_search()
    {
        ...
        $resultlist   = $this->patient_model->searchipd_datatable($where_condition, $this->hospital_id, $filters);
        $total_result = $this->patient_model->searchipd_datatable_count($this->hospital_id, $filters);
        $data         = [];
    ...
    // print_r($resultlist);
        foreach ($resultlist as $result_value) {
            ...
            // Action buttons
            $action = "<div class='rowoptionview'>";
            $action .= "<a href='" . base_url('hospital/patient/ipdprofile/' . $result_value->id) . "' class='btn btn-default btn-xs' data-toggle='tooltip' title='" . $this->lang->line('show') . "'>
                            <i class='fa fa-reorder' aria-hidden='true'></i>
                        </a>";
            $action .= "</div>";
    
            // Patient profile link
            $first_action = "<a href='" . base_url('hospital/patient/ipdprofile/' . $result_value->id) . "'>";
            $nestedData[] = $first_action . htmlspecialchars($result_value->patient_name) . "</a> " . $action;
            $nestedData[] = isset($result_value->ipd_no) ? $result_value->ipd_no : 'N/A';
            $nestedData[] = htmlspecialchars($result_value->patient_unique_id);
            $nestedData[] = htmlspecialchars($result_value->patient_cnic);
            $nestedData[] = htmlspecialchars($result_value->gender);
            $nestedData[] = htmlspecialchars($result_value->mobileno);
            $nestedData[] = htmlspecialchars($result_value->created_at);
            $nestedData[] = $result_value->moved_to_ipd ? 'Came From OPD' : 'Direct IPD';


    
            $data[] = $nestedData; // Add row to data array
        }
    ...
    public function discharged_search()
    {
        ...
        $resultlist = $this->patient_model->searchdischarged_datatable($where_condition,$this->hospital_id);
        $total_result = $this->patient_model->searchdischarged_datatable_count($where_condition);
        // print_r($resultlist);exit;
        $data = array();
        
        // print_r($resultlist);exit;
        $data = []; // Ensure $data is initialized before use

        foreach ($resultlist as $result_key => $result_value) {
            $action = "<div class='rowoptionview'>";

            $action .= "<a href=" . base_url() . 'hospital/patient/ipdprofile/' . $result_value->id . " class='btn btn-default btn-xs'  data-toggle='tooltip' title='" . $this->lang->line('show') . "'><i class='fa fa-reorder' aria-hidden='true'></i></a>";
            ...
            $nestedData[] = $result_value->gender;
            $nestedData[] = $result_value->mobileno;
            $nestedData[] = !empty($result_value->discharged_date) ? date('Y-m-d', strtotime($result_value->discharged_date)) : null;
            $nestedData[] = $result_value->discharged_type;
            $nestedData[] = $result_value->remarks;
    ...
    public function discharged_patients()
    {
        ...
        $this->session->set_userdata('top_menu', 'IPD_in_patient');
        $this->load->view('layout/user/header');
        $this->load->view('store/patient/dischargedPatients.php');
        $this->load->view('layout/user/footer');
    }
    ...
    public function profile($id)
    {
        ...
        $this->load->view("layout/user/header");
        $this->load->view("store/patient/profile", $data);
        $this->load->view("layout/user/footer");
    }
    ...
    public function ipdprofile($id, $ipdid = '', $active = 'yes')
    {
        ...
        if (!empty($id)) {
            ...
            $billDetails                 = $this->payment_model->getBillDetails($id, $ipdid);
            $paid_amount                 = $this->payment_model->getPaidTotal($id, $ipdid);
            $data["paid_amount"]         = $paid_amount["paid_amount"];
            $balance_amount              = $this->payment_model->getBalanceTotal($id, $ipdid);
            $packages                    = $this->charge_model->getPackageDetails($charges);
            $assignedPackages            = $this->charge_model->getAssingedPackages($id, $ipdid);

            $data["balance_amount"]      = $balance_amount["balance_amount"];
            $data["payment_details"]     = $paymentDetails;
            $data["bill_details"]        = $billDetails;
            ...
            $data["assigned_packages"]   = $assignedPackages;
            $data["cheques"]             = $this->patient_model->getTPACheques($id);
            $data['roles']               = $this->role_model->get();
            ...
            $data['bills']                       = $this->pharmacy_model->getBills($this->hospital_id,$this->store_id,$ipdid);
        }
        $data['ipdconferences']  = $this->conference_model->getconfrencebyipd($doctorid, $id, $ipdid);

        $this->load->view("layout/user/header");
        $this->load->view("store/patient/ipdprofile", $data);
        $this->load->view("layout/user/footer");
    }
    ...
    public function getIpdDetails()
    {
        ...
        $result['discharge_date'] = date('Y-m-d H:i:s');
        echo json_encode($result);
    }
    ...
    public function update()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->patient_model->update_patient($patient_data);
    
            $comments = "Updated patient: Name - " . $this->input->post('name') . ", Father Name - " . $this->input->post('father_name') . ", MR# - " . $this->input->post('mr_no');
    
            $array = array('status' => 'success', 'message' => 'Record updated successfully');
        }
    ...
    public function editDiagnosis()
    {
        ...
        $result["report_date"] = date('Y-m-d', strtotime($result['report_date']));
        echo json_encode($result);
    }
    ...
    public function moveipd($id)
    {
        ...
        $appointment_date = date('Y-m-d H:i:s', strtotime($appointment_details['appointment_date']));
        ...
        $updateData = array('id' => $patient_id,'is_ipd'=>'yes', 'moved_to_ipd' => 1, 'discharged' => 'no');
        $this->patient_model->add($updateData);
        $ipdid = $this->patient_model->add($ipdpatient_data);
        redirect("hospital/patient/ipdsearch/");
    }
    ...
    public function getorganizationChargeRadio()
    {
        if (!$this->rbac->hasPrivilege('patient', 'can_view')) {
            access_denied();
        }
        ...
        $result         = $this->patient_model->getorganizationChargeRadio($orgId, $testId);
        $data['result'] = $result;
        echo json_encode($result);
    }

    public function getorganizationChargePatho()
    {
        if (!$this->rbac->hasPrivilege('patient', 'can_view')) {
            access_denied();
        }
        ...
        $orgId   = $this->input->post("organisation");
        $testId   = $this->input->post("testId");
        $result         = $this->patient_model->getorganizationChargePatho($orgId, $testId);
        $data['result'] = $result;
        echo json_encode($result);
    }
    ...
    public function checkExistPatient($patient_id, $date, $hospital_id = null)
{
    $this->db->select('id');
    ...
    if ($query->num_rows() > 0) {
        return $query->row()->id; // Return OPD ID if visit exists
    } else {
        return false; // Return false if visit does not exist
    }
}
    ...
    public function patient_search()
    {
        ...
        $resultlist   = $this->patient_model->searchpatient_datatable($hospital_id,$store_id);
        $total_result = $this->patient_model->searchpatient_datatable_count($hospital_id,$store_id);
        $data         = array();
    ...
    public function assignMedicine(){
        ...
        $data["doctors"]          = $doctors;
        $data['medicineCategory'] = $this->medicine_category_model->getMedicineCategory();
        $data['hospitalStores'] = $this->store_model->getHospitalDepartmentStores('Department', $this->hospital_id);
        $data['medicineName']     = $this->pharmacy_model->getMedicineName();
        $patients                 = $this->patient_model->getPatientListall(null,null,$this->hospital_id,$this->store_id);
        $data["patients"]         = $patients;
        $data["marital_status"]   = $this->marital_status;
        $data["bloodgroup"]       = $this->blood_group;
        $data['dosage']   = $this->medicine_dosage_model->getMedicineDosage();
        $data['instruction']   = $this->medicine_dosage_model->getMedicineInstruction();
        $data['departments']         = $this->staff_model->getDepartmentsbyUser();
        if($data['departments'] == null){
            $data['departments'] = $this->staff_model->getDepartmentbyUser(); 
        }

        $this->load->view('layout/user/header');
        $this->load->view('store/patient/assignMedicine', $data);
        $this->load->view('layout/user/footer');
    }
    ...
    public function assignedList(){
        ...
        $medicineCategory             = $this->medicine_category_model->getMedicineCategory();
        $data["medicineCategory"]     = $medicineCategory;
        $result                       = $this->pharmacy_model->getBills($this->hospital_id,$this->store_id,null);
        $data['bills']               = $result;
    ...
    //    echo "<pre>"; print_r($data);exit;
        ...
        $this->load->view('layout/user/header');
        $this->load->view('store/patient/index',$data);
        $this->load->view('layout/user/footer');
    }
    ...
    public function searchPatient() {
        ...
        $hospital_id = $this->input->get('hospital_id');  // Optional
        $store_id = $this->input->get('store_id');  // Optional
    
        // Call the model function to search patients
        $patients = $this->patient_model->getPatientListall('', '', $hospital_id, $store_id, $searchTerm);
    
        // Return the filtered patient list as a JSON response
        echo json_encode($patients);
    }
    public function addBill()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $bill_no = $this->pharmacy_model->generateBillNumber(); // Assume this function generates a unique bill number
            $bill_date = $this->input->post('date');
            $visit_id = $this->input->post('visit_id');
            // Bill Data
            $data = array(
                'date'         => date('Y-m-d H:i:s', strtotime($bill_date)),
                'bill_no'      => $bill_no,  // ✅ Auto-generated
                ...
                'hospital_id'  => $this->hospital_id,
                'store_id'     => $this->store_id,
                'department_id'=> $this->input->post('department_id') ? $this->input->post('department_id') : 0,
                'generated_by' => $this->user_id,
                'opd_ipd_no'=>$visit_id
            );
    
            $insert_id = $this->pharmacy_model->addBill($data);
            ...
            if ($insert_id) {
                ...
                $medicine_batch_details_id = $this->input->post('id'); // ✅ Fetch batch details
                ...
                foreach ($medicine_name as $key => $mvalue) {
                    ...
                    if (!isset($expiry_date[$key]) || empty($expiry_date[$key])) {
                        $msg = 'Expiry date is missing for medicine: ' . $mvalue;
                        echo json_encode(['status' => 'fail', 'error' => '', 'message' => $msg]);
                        return;
                    }
    
                    // ✅ Validate Expiry Date Format (Month/Year)
                    ...
                    $month_number = $this->convertMonthToNumber($explore[0]); // ✅ Convert Month to Number
                    $insert_date = $explore[1] . "-" . $month_number . "-01";
    
                    // ✅ Add Batch Details
                    $details = array(
                        'medicine_name'          => $medicine_name[$key],
                        'pharmacy_bill_basic_id' => $insert_id,
                        'medicine_category_id'   => $medicine_category_id[$key],
                        'batch_no'               => $batch_no[$key],  // ✅ Include batch number
                        'expire_date'            => $expiry_date[$key], // ✅ Include expiry date
                        'quantity'               => $quantity[$key],
                        'sale_price'             => $sale_price[$key],
                        'amount'                 => $amount[$key],
                        'dosage_id'=>$dosage[$key],
                        'instruction_id'=>$instruction[$key],

                    );
    
                    // ✅ Update Available Quantity
                    $available_quantity[$key] = $total_quantity[$key] - $quantity[$key];
                    $update_quantity = array(
                        'id'                 => $medicine_batch_details_id[$key],
                        'available_quantity' => $available_quantity[$key],
                    );
                    $this->pharmacy_model->availableQty($update_quantity);
                    $data         = array('opd_id' => $visit_id, 'visit_id' => $visit_id, 'medicine' =>  $medicine_name[$key], 'medicine_category_id' => $medicine_category_id[$key]);
                    $data_array[] = $data;
                    $data1[] = $details;
                }
                $this->patient_model->add_prescription($data_array);
                ...
                $this->pharmacy_model->addBillBatch($data1);
            }
    ...
    /**
     * Custom validation function to check unique bill_no
     */
    public function check_unique_bill_no($bill_no)
    {
        $this->db->where('bill_no', $bill_no);
        $query = $this->db->get('pharmacy_bill_basic');
    
        if ($query->num_rows() > 0) {
            $this->form_validation->set_message('check_unique_bill_no', 'The Bill No must be unique.');
            return false;
        }
        return true;
    }
    ...
    public function bill_search()
    {
        ...
        foreach ($resultlist as $result) {
            ...
            $nestedData   = array();
            // $action       = "<div class='rowoptionview'>";
            $action .= "<a href='#' onclick='viewDetail(" . $result['id'] . ")' class='btn btn-default btn-xs' data-toggle='tooltip' title='" . $this->lang->line('show') . "'><i class='fa fa-reorder'></i></a>";
    ...
    public function viewMedicineDetail(){
        ...
        $data['request'] = $this->pharmacy_model->getBill($id,$this->hospital_id,$this->store_id,$opd_id);
        // echo '<pre>'; print_r($data); exit;
        $bill_id = isset($data['request']->id) ? $data['request']->id : null;
        $data['details'] = $this->pharmacy_model->getPharmacyBillDetails($bill_id);
        $this->load->view('layout/user/header');
        $this->load->view('store/patient/viewAssignedMedicine',$data);
        $this->load->view('layout/user/footer');
    ...
    public function ipdPatientCreate()
    {
        $ipd_data         = $this->session->flashdata('ipd_data');
        $data['ipd_data'] = $ipd_data;
        $ipd = $this->input->get('is_ipd'); // Get IPD flag from request
        $data['ipd'] = $ipd;
        ...
        // 🔹 **Fetch ICU or Emergency records from `emergency_icu` table**
        if ($ipd) {
            // Fetch ICU records from emergency_icu table
            $data['resultlist'] = $this->emergency_icu_model->getICURecords($this->hospital_id);
        } else {
            // Fetch Emergency records from emergency_icu table
            $data['resultlist'] = $this->emergency_icu_model->getEmergencyRecords($this->hospital_id);
        }
    
        $data['organisation'] = $this->organisation_model->get();
    
        $this->load->view('layout/user/header');
        $this->load->view('store/patient/create.php', $data);
        $this->load->view('layout/user/footer');
    }
    ...
    public function getPatientDetail() {
        ...
        // Set validation rules
            ...
            // Get input values
            $cnic = $this->input->post('cnic');
            $mrno = $this->input->post('mrn');
            $other_nationality = $this->input->post('other_nationality');
    
            // Load model
            $this->load->model('Patient_model');
            $response = $this->Patient_model->getPatientDetail($cnic, $mrno,$other_nationality,$this->hospital_id);
    ...
    public function add()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $date             = date('Y-m-d H:i:s', $this->customlib->datetostrtotime($appointment_date));
            $insert_id        = $this->input->post('patient_id');
            $checkOpdPatient = $this->checkExistPatient($insert_id, $date);
            if ($checkOpdPatient == false) {
                $msg = array(
                    'appointment_date' => "visit already created against pateint! "
                );
                $array = array('status' => 'fail', 'error' => $msg, 'message' => '');
                // echo json_encode($array);
                ///exit;
                ...
                $chargesCommission = $this->staff_model->consultantChargesStaffCommission($doctor_id);
                if (!empty($chargesCommission)) {
                    $commission_month = date('m', strtotime($date));
                    $commission_year = date('Y', strtotime($date));
                    if ($chargesCommission['doctor_percentage_type'] == 'fixed') {
            ...
            // Fetch the highest token number for the given doctor and date
            $this->db->select_max('token_number');
            $this->db->where('consultant_id', $doctor_id);
            $this->db->where('date', date('Y-m-d', strtotime($date)));
            $query = $this->db->get('daily_patient_tokens');
            $result = $query->row();
            // Determine the new token number
            $new_token_number = ($result && $result->token_number) ? $result->token_number + 1 : 1;
            // Update the token number in the token_data array
            $token_data['token_number'] = $new_token_number;
    ...
    public function add_opdpatient() {
        // Set validation rules
        $this->form_validation->set_rules('name', 'Name', 'trim|required|min_length[3]');
        $this->form_validation->set_rules('father_name', 'Father Name', 'trim|required|min_length[3]');
        ...
        $cnic = $this->input->post('cnic');
        if ($this->patient_model->check_cnic_exists($cnic)) {
            echo json_encode([
                'status' => 'error',
        ...
        $data = [
            ...
            'father_name' => $this->input->post('father_name'),
            'mobileno' => $this->input->post('mobile'),
            'patient_cnic' => $this->input->post('cnic'),
            'other_nationality' => $this->input->post('other_nationality'),
            'age' => $this->input->post('age'),
            'gender' => $this->input->post('gender'),
            'ward' => $this->input->post('ward'),
    ...
    public function getPrescriptionmanual($id, $opdid,$emg='')
    {
        ...
        $data["print_details"] = $this->printing_model->get('', 'opdpre',$this->hospital_id);
        $data["result"]        = $result;
        ...
        if($emg=='emg'){
            $prescription_list     = $this->prescription_model->getPrescriptionByEMG($result['opd_id']);
        }else{
            $prescription_list     = $this->prescription_model->getPrescriptionByOPD($result['id']);
        }
...
// echo "<pre>";  print_r($data);exit;
    ...
    public function printBillInvoice()
    {
        ...
        if($emg_id=='emg'){
            ...
            $token_data   = $this->patient_model->getTokenNo($date, $id);
            $data['token_number'] = $token_data['token_number'];
            $data['generated_by'] = $token_data['generated_by_name'];

        }
        $data['hospital_name'] =     $this->session->userdata('hospital')['hospital_name'];        ;
    ...
    public function dischargePatient()
{
    ...
    if (!empty($patient_id)) {
        $update_data = array(
            'discharged' => 'yes',
            'discharged_type'=>$this->input->post('discharge_type'),
            'discharged_date' => $this->input->post('date'),
            'remarks' => $this->input->post('remarks'),
        );

        $this->db->where('id', $patient_id);
        if ($this->db->update('patients', $update_data)) {
